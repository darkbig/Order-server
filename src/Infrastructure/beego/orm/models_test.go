...
type Post struct {
	...
	Updated time.Time `orm:"auto_now"`
	Tags    []*Tag    `orm:"rel(m2m);rel_through(Infrastructure/beego/orm.PostTags)"`
}
...
type Permission struct {
	ID     int `orm:"column(id)"`
	Name   string
	Groups []*Group `orm:"rel(m2m);rel_through(Infrastructure/beego/orm.GroupPermissions)"`
}
...
go get -u Infrastructure/beego/orm
...
go test -v Infrastructure/beego/orm
